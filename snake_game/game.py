...
class game_play:
    ...
    def random_food_positions(self):
        ...
        for _ in range(num_apples):
            ...
            food_positions.append(pygame.Vector2(x, y))
    ...
    def setup(self):
        ...
        self.primary_snake = Snake(GREEN, WIDTH, HEIGHT, BLOCK_SIZE)

        if self.is_multiplayer:
            self.secondary_snake = Snake(BLUE, WIDTH, HEIGHT, BLOCK_SIZE)
    ...
    def draw_nfl_scores(self):
        ...
        pygame.draw.rect(self.screen, BLACK, (0, 0, WIDTH, 50))  # Bannière noire pour les scores
    ...
    def play(self):
        ...
        while running:
            ...
            # Utilisation des agents pour contrôler les directions des serpents
            new_direction_primary = self.primary_agent(self.primary_snake, self.food_positions[0])
            self.primary_snake.change_direction(new_direction_primary)

            if self.is_multiplayer:
                new_direction_secondary = self.secondary_agent(self.secondary_snake, self.food_positions[0])
            ...
            if self.primary_snake.collision_with_self() or self.primary_snake.out_of_bounds(WIDTH, HEIGHT):
                if self.is_multiplayer:
                    # Si le serpent principal sort, le second est le gagnant
                    self.winner = self.secondary_agent_name
                running = False

            if self.is_multiplayer and (self.secondary_snake.collision_with_self() or self.secondary_snake.out_of_bounds(WIDTH, HEIGHT)):
                # Si le second serpent sort, le premier est le gagnant
                self.winner = self.primary_agent_name
                running = False
